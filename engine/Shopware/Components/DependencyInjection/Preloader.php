...
{
    public static function generate(string $file, array $classes, array $includes): void
    {
        $content = '<?php
...
';
        ...
        $requireTpl = 'require \'%s\';' . PHP_EOL;
        $classTpl = '$classes[] = \'%s\';' . PHP_EOL;

        foreach ($includes as $include) {
            $content.= \sprintf($requireTpl, $include);
        }

        $content .= PHP_EOL;

        foreach ($classes as $class) {
            $content.= \sprintf($classTpl, $class);
        }

        $content .= 'Preloader::preload($classes);';
